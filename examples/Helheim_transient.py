...
pdeweights = float(sys.argv[4]) if len(sys.argv) > 4 else 10
epochs = int(sys.argv[5]) if len(sys.argv) > 5 else 400000
...
# output folder
outputFileName=f"{filename}_epochs{epochs}_sigma{sigma:.0f}_nff{num_fourier_feature}_w1e{pdeweights}"
...
# physics
hp["equations"] = {"Thickness":{"pde_weights":[10**pdeweights]}}
