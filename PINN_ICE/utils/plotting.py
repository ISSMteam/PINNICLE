...
def plot_solutions(pinn, path="", X_ref=None, sol_ref=None, cols=None, **kwargs):
    ...
    # generate 200x200 mesh on the domain
        X, Y = np.meshgrid(np.linspace(pinn.param.nn.input_lb[0], pinn.param.nn.input_ub[0], 200),
                np.linspace(pinn.param.nn.input_lb[1], pinn.param.nn.input_ub[1], 200))
